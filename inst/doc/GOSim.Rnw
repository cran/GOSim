...
Since version 1.2 \emph{GOSim} also contain Schlicker et al.'s  and Li et al.'s GO term similarity measures \cite{Schlicker2006GOFuncSim, Li2009GOSimLinAdaption}, which are both an adaption of Lin's similarity measure. Moreover, the graph information content similarity by Pesquita et al. has been implemented \cite{Pesquita2007GOSim}.
<<results=verbatim>>=
