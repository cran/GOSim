...
%\VignetteDepends{igraph, topGO, annotate, cluster}
...
IC[c("GO:0007166","GO:0007267","GO:0007584","GO:0007165","GO:0007186")]/max(IC) 
@
...
<<GOExample,eval=FALSE,results=hide,fig=TRUE,eps=FALSE,include=FALSE>>=
library(igraph)
...
Since version 1.2 \emph{GOSim} also contain Schlicker et al.'s  and Li et al.'s GO term similarity measures \citep{Schlicker2006GOFuncSim, Li2009GOSimLinAdaption}, which are both an adaption of Lin's similarity measure. Moreover, the graph information content similarity by Pesquita et al. has been implemented \citep{Pesquita2007GOSim}.
<<results=verbatim>>=
...
<<results=verbatim>>=
getGeneSim(c("8614","2852"),similarity="OA",similarityTerm="Lin",verbose=FALSE)
@
...
Since version 1.1 \emph{GOSim} also offers the possibility of a GO enrichemnt analysis. Suppose, we may now want to get a clearer picture of the genes involved in cluster 1. For this purpose we use the topGO tool \citep{Alexa2006topGO}.
<<results=verbatim, eval=FALSE>>=
gomap <- get("gomap",env=GOSimEnv)
allgenes = unique(c(sample(names(gomap), 1000), genes)) # suppose these are all genes
