 ...
 ** PURPOSE.  
 ** 
 ** modified in order to handle real valued problems by Holger Froehlich, 2004 
 **
 ********************************************************************
 ********************************************************************/
...
void hungarian_print_assignment(hungarian_problem_t* p) {
  hungarian_print_matrixInt(p->assignment, p->num_rows, p->num_cols) ;
}

void hungarian_print_costmatrix(hungarian_problem_t* p) {
  hungarian_print_matrixDouble(p->cost, p->num_rows, p->num_cols) ;
}
